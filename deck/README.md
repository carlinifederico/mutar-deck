...
## Estructura — 20 frames en 8 actos + 3 interludios
...
**Interludio B · el carrete (frame 03 → 04).** Un feed de fotos personales que
...
mide 375vh y adentro un stage pegajoso desplaza las tarjetas. El progreso se
parte en un tramo por step — la primera mitad lo sostiene, la segunda viaja al
siguiente — así los cinco tienen su momento y ninguno se saltea. Cada step
tiene ícono, color propio y se apaga cuando no es el activo.
...
- **Bagel Fat One** — solo numerales y palabras sueltas gigantes (divisor,
  números de step, marquee). Es hermosa pero ilegible en frases largas.
...
js/motion.js       reveals, split, marquees, carrete, track, auras, cursor
...
3. **`overflow: hidden` sólo en los interludios que lo necesitan.** En el
   carrete convertía al interludio en contenedor de scroll y anulaba el
   `sticky` del stage.
